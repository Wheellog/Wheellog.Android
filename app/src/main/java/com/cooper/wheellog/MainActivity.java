        ...
        public void onReceive(Context context, Intent intent) {
            switch (intent.getAction()) {
                ...
                case Constants.ACTION_ALARM_TRIGGERED:
                    ...
                    float alarmValue = intent.getFloatExtra(Constants.INTENT_EXTRA_ALARM_VALUE, 0f);
                    if (alarmType < 4) {
                        showSnackBar(getResources().getString(R.string.alarm_text_speed)+String.format(": %0.2f",alarmValue), 3000);
                    }
                    if (alarmType == 4) {
                        showSnackBar(getResources().getString(R.string.alarm_text_current)+String.format(": %0.2f",alarmValue), 3000);
                    }
                    if (alarmType == 5) {
                        showSnackBar(getResources().getString(R.string.alarm_text_temperature)+String.format(": %0.2f",alarmValue), 3000);
                    }
                    if (alarmType == 6) {
                        showSnackBar(getResources().getString(R.string.alarm_text_pwm)+String.format(": %0.2f",alarmValue), 3000);
                    }
                    break;
            }
        }
